...
import { getQRToken } from "../../../../api/eventParticipation";
...
const QRCodeModal: React.FC<QRCodeModalProps> = ({ session, onClose }) => {
  ...
  useEffect(() => {
    const fetchToken = async () => {
      ...
      setError("");
      try {
        const token = await getQRToken(session.sessionId);
        setQrToken(token);
      } catch (err: any) {
  ...
  return (
    ...
    >
      <motion.div
        className="w-full max-w-xs xs:max-w-sm md:max-w-md rounded-xl xs:rounded-2xl bg-[#170657] border border-white/10 shadow-2xl overflow-hidden"
      ...
      >
        <div className="p-4 xs:p-5 sm:p-6">
          ...
          {/* QR code display */}
          {loading && (
            <div className="flex flex-col items-center py-6 xs:py-8">
              ...
              <p className="text-white/50 text-xs xs:text-sm">
                Loading QR code...
              </p>
            </div>
          )}
          ...
          {!loading && qrToken && (
            <div className="flex flex-col items-center py-3 xs:py-4">
              <div className="bg-white rounded-lg xs:rounded-xl p-1.5 xs:p-2 mb-3 xs:mb-4">
                <QRCodeCanvas
                  value={`https://csps.com/${qrToken}`}
                  size={280}
                  level="L"
                  marginSize={4}
              ...
              <p className="text-white/40 text-xs xs:text-sm text-center max-w-[200px] xs:max-w-[250px]">
                Show this QR code to the event organizer for attendance
              </p>
            </div>
          )}
          ...
          <button
            onClick={onClose}
            className="w-full mt-3 xs:mt-4 sm:mt-5 py-2 xs:py-2.5 sm:py-3 bg-white/5 text-white/60 rounded-lg xs:rounded-xl text-xs xs:text-sm font-medium hover:bg-white/10 transition-colors"
          >
            Close
          </button>
        </div>
